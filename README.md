...
## Структура
- `include/ui_primitives.h` и `src/ui_primitives.c` — потокобезопасный контекст, framebuffer, очереди событий (сенсор, клавиатура), рисование прямоугольников и текст через шрифт BareUI, API управления шрифтами и событиями. `include/ui_core.h` теперь просто включает этот слой и будет расширяться виджетным API поверх примитивов.
