...
- `include/ui_widget.h` и `src/ui_widget.c` — начальная абстракция виджетов/контейнеров: иерархия, bounds, отрисовка и маршрутизация событий поверх примитивов, а также гибкий `ui_style_t` для настройки цветов, отступов, теней и дополнительных свойств. `include/ui_core.h` теперь просто включает widget-слой, чтобы UI-слой можно было расширять.
